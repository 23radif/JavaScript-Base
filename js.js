...
const map = {
  ...
  render(snakePointsArray, foodPoint) {
    ...
    this.usedCells.push(foodCell); //Здесь была ошибка: ...(foodPoint)
  },
};
 ...
 * @property {int} maxX - Максимально возможная координата по X
 * @property {int} maxY - Максимально возможная координата по Y
 * @property {settings} settings Настройки игры.
 */
const snake = {
  body: null,
 ...
 * @property {number} score Количество съеденной еды. (Счет игры)
...
const game = {
   ...
   */
  init(userSettings) {
    //1. Когда мы инициализируем игру, нужно также инициализировать счетчик.
    this.score.init();
   ...
   */
  reset() {
    // 2. Когда мы ставим игру в начальное состояние, мы должны сбросить в ноль наш счетчик.
    this.score.drop();
  ...
  tickHandler() {
    ...
    if (this.food.isOnPoint(this.snake.getNextStepHeadPoint())) {
      ...
      this.snake.growUp();
      // 2. Когда пользователь съел еду мы должны к нашему счету прибавить единицу.
  ...
  canMakeStep() {
    ...
    return !this.snake.isOnPoint(nextHeadPoint)/* &&
      ...
      nextHeadPoint.x >= 0 &&
      nextHeadPoint.y >= 0*/; //Убираем проверку для перемещения за стенку
  },
};
